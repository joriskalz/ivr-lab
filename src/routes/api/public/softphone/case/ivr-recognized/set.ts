...
import { setSoftphoneCaseIvrRecognizedPayloadForCorrelationCode } from '@/features/softphone/server/case-store'
...
import { parseSoftphoneCaseDataRecognizedPayload, readJsonBody } from '@/features/softphone/server/parsers'

getSoftphoneServerConfig()

export const Route = createFileRoute('/api/public/softphone/case/ivr-recognized/set')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const parsedPayload = parseSoftphoneCaseDataRecognizedPayload(body.value)

        if (!parsedPayload.ok) {
          return jsonResponse({ error: parsedPayload.issues.join(' ') }, { status: 400 })
        }

        return jsonResponse(setSoftphoneCaseIvrRecognizedPayloadForCorrelationCode(accessResult.correlationCode, parsedPayload.value))
      },
    },
  },
})
