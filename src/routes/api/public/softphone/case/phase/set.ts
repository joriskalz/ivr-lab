...
import { setSoftphoneCasePhasePayloadForCorrelationCode } from '@/features/softphone/server/case-store'
...
import { parseSoftphoneCasePhasePayload, readJsonBody } from '@/features/softphone/server/parsers'

getSoftphoneServerConfig()

export const Route = createFileRoute('/api/public/softphone/case/phase/set')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const parsedPayload = parseSoftphoneCasePhasePayload(
          body.value,
        ...
        return jsonResponse(setSoftphoneCasePhasePayloadForCorrelationCode(accessResult.correlationCode, parsedPayload.value))
      },
    },
  },
})
