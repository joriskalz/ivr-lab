...
import { setSoftphoneCaseDebugInformationPayloadForCorrelationCode } from '@/features/softphone/server/case-store'
...
import { parseSoftphoneDebugInformationPayload, readJsonBody } from '@/features/softphone/server/parsers'

getSoftphoneServerConfig()

export const Route = createFileRoute('/api/public/softphone/case/debug-information/set')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const parsedPayload = parseSoftphoneDebugInformationPayload(body.value)
        ...
        return jsonResponse(
          setSoftphoneCaseDebugInformationPayloadForCorrelationCode(accessResult.correlationCode, parsedPayload.value),
        )
      },
    },
  },
